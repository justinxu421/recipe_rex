...
def process_embeddings(file, save = False):
    df = pd.read_csv(f'../clean_data/{file_name}.csv', index_col = 'url')    
    df = df.dropna()
    ...
    if save:
        print(f'saving to clean_data/{file_name}_scaled.csv\n')
        df_scaled.to_csv(f'../clean_data/{file_name}_scaled.csv')
...
def get_embeddings(all_dfs_nutrient, load = True, save = False):
    ...
    if save:
        print(f'saving to clean_data/{file_name}.csv\n')
        embeddings_df.to_csv(f'../clean_data/{file_name}.csv')
