...
# drop nas
# scale to standard
def process_embeddings(folder, prefix, save = False):
    df = pd.read_csv(f'../clean_data/{folder}/embeddings_{prefix}.csv', index_col = 'url')    
    df = df.dropna()
    ...
    if save:
        print(f'saving to clean_data/{folder}/embeddings_{prefix}_scaled.csv\n')
        df_scaled.to_csv(f'../clean_data/{folder}/embeddings_{prefix}_scaled.csv')
        
    return df_scaled

def get_embeddings(all_dfs_nutrient, folder, prefix, load = True, save = False):
    ...
    if save:
        print(f'saving to clean_data/{folder}/embeddings_{prefix}.csv\n')
        embeddings_df.to_csv(f'../clean_data/{folder}/embeddings_{prefix}.csv')
...
def main(folder, prefix):
    ...
    # save the embeddings
    embeddings_df = get_embeddings(all_dfs_nutrient, folder, prefix, load = True, save = True)
    
    # also save scaled version
    embeddings_df_scaled = process_embeddings(folder, prefix, save = True)

if __name__ == "__main__":
    main('all', 'all')
    main('mains', 'mains_filter')
    main('desserts', 'desserts_filter')
    main('sides', 'sides_filter')
